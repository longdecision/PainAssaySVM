# -*- coding: utf-8 -*-
"""
svm20190103CCI_generic_directory.py
5/7/2019 modified from svm20190103CCI.py to use default directory and trim unused code
...
def pipeline(df_settings):
    ...
    # Common settings
    data_path = '.'  
    data_file = ['C57 male.csv', 'CCI injured paw.csv', 'CCI uninjured paw.csv']
    out_path = 'PCA4CCI'
    ...
    # plot output
    # separated by weeks
    timepoints = list(df['TIME'].unique())
    genotype = list(df['Genotype (I.D.)'].unique())
    ncols = len(timepoints)
    n_data = len(genotype)
    fig, ax = plt.subplots(nrows = n_data, ncols = ncols)
    fig.set_size_inches(w = ncols* 2.4, h = n_data*2.5)
    groups = df.groupby(by = ['Genotype (I.D.)', 'TIME'])
    print(genotype, timepoints)
    for name, group in groups:
        irow = genotype.index(name[0])
        icol = timepoints.index(name[1])
        stim = plot_data(group, ax[irow, icol], f, type4train)
        set_font_label(ax[irow, icol], 'P_pain', '', [' '.join(name), 9])
        ax[irow, icol].set_xticklabels(stim, rotation = 45)
...
def main():
    ...
    settings_path = '.'   
    setting_file = os.path.join(settings_path, 'plots_20181214CCI.csv')
...
def plot_data(df, ax, f, type4train):
    ...
    for i, stim_type in enumerate(stim):
        ...
        if f + ' ' + str(stim_type) in type4train:
            ax.plot(x2plot, prob, marker = 'o', linestyle='None', markerfacecolor = 'None', markeredgecolor = 'r', clip_on = False)
        else:
            ax.plot(x2plot, prob, marker = 'o', linestyle='None', markerfacecolor = 'None', markeredgecolor = 'k', clip_on = False)
        ax.plot([i-0.3, i+0.3], [prob.mean(), prob.mean()], 'k-', marker='None', linestyle = '-', linewidth = 2 )
